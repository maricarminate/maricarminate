# 👋 Hello! I'm Mariana Carminate

🧠 **Philosopher** | 💻 **Full Stack Developer** | 📊 **Postgraduate in Big Data, ML & IA and Cybersecurity**

[![LinkedIn](https://img.shields.io/badge/LinkedIn-blue?style=flat&logo=linkedin)](https://linkedin.com/in/yourprofile)
[![Email](https://img.shields.io/badge/Email-red?style=flat&logo=gmail)](mailto:your.email@gmail.com)
[![Portfolio](https://img.shields.io/badge/Portfolio-black?style=flat)](https://yourportfolio.com)

> **"From critical thinking to secure artificial intelligence"** 🧠🔐🤖

---

## 🚀 About Me

I'm a multidisciplinary developer with a unique combination of skills ranging from Philosophy to Machine Learning and Cybersecurity.

### 🎓 Education
- 🧠 **Bachelor's Degree in Philosophy (UFJF)** ✅
- 📊 **Postgraduate in Big Data (Faculdade Iguaçu)** ✅
- 🔐 **Postgraduate in Cybersecurity (Faculdade Iguaçu)** 🔄
- 🤖 **Postgraduate in ML & AI (Instituto Prominas)** 🔄
- 💻 **Full Stack Developer Certification (Codi Academy)** ✅

### 💡 What Sets Me Apart

**Philosophy taught me:**
- 🧠 Critical thinking about complex problems
- 🤔 Question assumptions in algorithms
- ⚖️ Develop AI with ethical awareness
- 📢 Communicate technical ideas with clarity

**Technology allows me:**
- 🏗️ Build complete systems (Full Stack)
- 📈 Process data at scale (Big Data)
- 🧠 Create intelligent models (ML/AI)
- 🔒 Protect what I build (Cybersecurity)

### 🎯 Result
**Complete, secure, scalable, and ethical AI systems**

Currently focused on: **MLOps • Secure AI • Big Data ML • Explainable AI**

---

## 🛠️ Complete Tech Stack

### 💻 Full Stack Development
![JavaScript](https://img.shields.io/badge/-JavaScript-F7DF1E?style=flat&logo=javascript&logoColor=black)
![React](https://img.shields.io/badge/-React-61DAFB?style=flat&logo=react&logoColor=black)
![Node.js](https://img.shields.io/badge/-Node.js-339933?style=flat&logo=node.js&logoColor=white)
![Python](https://img.shields.io/badge/-Python-3776AB?style=flat&logo=python&logoColor=white)
![HTML5](https://img.shields.io/badge/-HTML5-E34F26?style=flat&logo=html5&logoColor=white)
![CSS3](https://img.shields.io/badge/-CSS3-1572B6?style=flat&logo=css3&logoColor=white)
![PostgreSQL](https://img.shields.io/badge/-PostgreSQL-336791?style=flat&logo=postgresql&logoColor=white)
![MongoDB](https://img.shields.io/badge/-MongoDB-47A248?style=flat&logo=mongodb&logoColor=white)

### 📊 Big Data & Analytics
![Apache Spark](https://img.shields.io/badge/-Apache%20Spark-E25A1C?style=flat&logo=apachespark&logoColor=white)
![Hadoop](https://img.shields.io/badge/-Hadoop-66CCFF?style=flat&logo=apachehadoop&logoColor=black)
![Kafka](https://img.shields.io/badge/-Kafka-231F20?style=flat&logo=apachekafka&logoColor=white)
![Pandas](https://img.shields.io/badge/-Pandas-150458?style=flat&logo=pandas&logoColor=white)
![NumPy](https://img.shields.io/badge/-NumPy-013243?style=flat&logo=numpy&logoColor=white)

### 🤖 Machine Learning & AI
![TensorFlow](https://img.shields.io/badge/-TensorFlow-FF6F00?style=flat&logo=tensorflow&logoColor=white)
![Keras](https://img.shields.io/badge/-Keras-D00000?style=flat&logo=keras&logoColor=white)
![Scikit-learn](https://img.shields.io/badge/-Scikit--learn-F7931E?style=flat&logo=scikit-learn&logoColor=white)
![PyTorch](https://img.shields.io/badge/-PyTorch-EE4C2C?style=flat&logo=pytorch&logoColor=white)

### 🔐 Cybersecurity & Tools
![Git](https://img.shields.io/badge/-Git-F05032?style=flat&logo=git&logoColor=white)
![Docker](https://img.shields.io/badge/-Docker-2496ED?style=flat&logo=docker&logoColor=white)
![VS Code](https://img.shields.io/badge/-VS%20Code-007ACC?style=flat&logo=visualstudiocode&logoColor=white)
![OWASP](https://img.shields.io/badge/-OWASP-000000?style=flat&logo=owasp&logoColor=white)

---

## 🌟 Featured Projects

### 🌸 Flower Classifier - Transfer Learning
![Python](https://img.shields.io/badge/-Python-3776AB?style=flat&logo=python&logoColor=white)
![TensorFlow](https://img.shields.io/badge/-TensorFlow-FF6F00?style=flat&logo=tensorflow&logoColor=white)
![Keras](https://img.shields.io/badge/-Keras-D00000?style=flat&logo=keras&logoColor=white)

Complete classification system for 5 types of flowers using Transfer Learning with MobileNetV2

**Highlights:**
- ✅ 92%+ accuracy on validation set
- ✅ Transfer Learning: Pre-trained MobileNetV2 on ImageNet
- ✅ Dataset: ~4,300 organized and cleaned images
- ✅ Features: Data augmentation, intelligent callbacks, professional visualizations
- ✅ Documentation: Complete README with step-by-step instructions

**Tech:** Python • TensorFlow • Keras • Computer Vision • Transfer Learning • Data Augmentation

### 🐱 Cat and Dog Classifier - CNN
![Python](https://img.shields.io/badge/-Python-3776AB?style=flat&logo=python&logoColor=white)
![TensorFlow](https://img.shields.io/badge/-TensorFlow-FF6F00?style=flat&logo=tensorflow&logoColor=white)
![Deep Learning](https://img.shields.io/badge/-Deep%20Learning-FF6F00?style=flat)

Convolutional Neural Network from scratch for binary classification

**Highlights:**
- ✅ Dataset: 25,000 images from Kaggle
- ✅ Architecture: CNN with 4 convolutional layers
- ✅ Accuracy: 85%+
- ✅ Complete pipeline: Preparation, training, evaluation, and testing

**Tech:** Python • TensorFlow • Keras • CNNs • Deep Learning • Image Classification

### 🚀 ExperimentVault - MLOps Platform
![FastAPI](https://img.shields.io/badge/-FastAPI-009688?style=flat&logo=fastapi&logoColor=white)
![React](https://img.shields.io/badge/-React-61DAFB?style=flat&logo=react&logoColor=black)
![PostgreSQL](https://img.shields.io/badge/-PostgreSQL-336791?style=flat&logo=postgresql&logoColor=white)

Comprehensive MLOps platform for experiment tracking, versioning, and comparison. Track ML experiments with full reproducibility and Git integration.

**Highlights:**
- ✅ Experiment tracking with automatic Git integration
- ✅ Side-by-side experiment comparison
- ✅ REST API with filtering and search
- ✅ Python client SDK with decorators
- ✅ Docker & Docker Compose support

---

## 🎯 Areas of Interest

### 🔥 Current Focus
- 🚀 **MLOps & Model Deployment**
- 🔐 **Secure AI Systems**
- 📊 **Scalable ML Pipelines**
- 🔍 **Explainable AI**
- 💾 **Big Data ML Integration**

### 🌱 Exploring
- 🎨 Generative AI
- 🤖 LLMs & Fine-tuning
- 🌐 Federated Learning
- 🛡️ AI Safety & Alignment
- ☁️ Cloud ML (AWS/GCP)

### ❤️ Passions
- ⚖️ **AI Ethics**
- 🔒 **Data Privacy**
- 🌍 **Responsible Tech**
- 💚 **Tech for Good**
- 🧠 **Philosophy of AI**

---

## 📚 Currently Studying
- 🔐 **Cybersecurity:** DevSecOps, Application Security, OWASP
- 🤖 **Advanced ML/AI:** Deep Learning, MLOps, Model Optimization
- ☁️ **Cloud Computing:** AWS, GCP, Azure ML
- 🚀 **Generative AI:** LangChain, Vector Databases, RAG

---

## 🏆 Achievements & Certifications

| Achievement | Status | Year |
|-------------|--------|------|
| 🎓 Bachelor's in Philosophy | ✅ | 2021 |
| 📊 Postgraduate Big Data | ✅ | 2025 |
| 💻 Full Stack Certification | ✅ | 2025 |
| 🔐 Postgraduate Cybersecurity | 🔄 | 2025 |
| 🤖 Postgraduate ML & AI | 🔄 | 2025 |

---

## 💼 Complementary Skills

### 🧠 Soft Skills (from Philosophy)
- 🎯 **Critical Thinking** - Deep analysis of complex problems
- 📖 **Communication** - Clear explanation of technical concepts
- ⚖️ **Ethics** - Responsible AI development
- 🔍 **Research** - Methodological investigation of solutions
- 🤝 **Collaboration** - Multidisciplinary teamwork

### 💻 Hard Skills
- ⭐ **Full Stack Development** - Frontend + Backend + Database
- ⭐ **Big Data Engineering** - Hadoop + Spark + ETL Pipelines
- ⭐ **Machine Learning** - Supervised + Unsupervised + Deep Learning
- ⭐ **Computer Vision** - CNNs + Transfer Learning + Object Detection
- ⭐ **MLOps** - Model Deployment + Monitoring + CI/CD

---

## 📊 GitHub Stats

[![GitHub Streak](https://streak-stats.demolab.com/?user=maricarminate)](https://git.io/streak-stats)

[![Top Langs](https://github-readme-stats.vercel.app/api/top-langs/?username=maricarminate&layout=compact)](https://github.com/anuraghazra/github-readme-stats)

---

## 💡 Why Philosophy + Tech?

| Philosophy Brings | Application in Tech |
|-------------------|---------------------|
| ✅ Critical thinking | → Questions algorithm assumptions |
| ✅ Argument analysis | → Responsible AI |
| ✅ Ethics and responsibility | → Data privacy and security |
| ✅ Bias identification | → ML bias detection |
| ✅ Clear communication | → Excellent documentation |

---

## 📫 Let's Connect!

Interested in discussing:
- 🤖 Machine Learning & AI
- 📊 Big Data
- 🔐 Cybersecurity
- 💻 Full Stack Development
- 🧠 AI Ethics
- 🌍 Tech for Good
- 📚 Philosophy of Technology

[![LinkedIn](https://img.shields.io/badge/LinkedIn-0077B5?style=for-the-badge&logo=linkedin&logoColor=white)](https://linkedin.com/in/yourprofile)
[![Email](https://img.shields.io/badge/Email-EA4335?style=for-the-badge&logo=gmail&logoColor=white)](mailto:your.email@gmail.com)
[![Portfolio](https://img.shields.io/badge/Portfolio-000000?style=for-the-badge&logo=github&logoColor=white)](https://yourportfolio.com)

---

## 💭 Work Philosophy

> **"I build technology that questions, learns, protects, and respects.**
> **From critical thinking to responsible artificial intelligence."**

---

⭐ If my projects helped you, consider giving a star!

Thanks for visiting my profile! 🌸
